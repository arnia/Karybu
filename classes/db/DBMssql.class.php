...
class DBMssql extends DB
{
    ...
    /**
     * prefix of Karybu tables(One more Karybu can be installed on a single DB)
     * @var string
     */
    var $prefix = 'k';
    var $param = array();
    ...
    function __connect($connection)
    {
        ...
        $result = @sqlsrv_connect(
            $connection["db_hostname"],
            array(
                'Database' => $connection["db_database"],
                'UID' => $connection["db_userid"],
                'PWD' => $connection["db_password"]
            )
        );
    ...
    {
        $query = sprintf(
            "insert into %ssequence (seq) values (ident_incr('%ssequence'))",
            $this->prefix,
            $this->prefix
        );
        $this->_query($query);
    ...
    {
        $query = sprintf(
            "select syscolumns.name as name from syscolumns, sysobjects where sysobjects.name = '%s%s' and sysobjects.id = syscolumns.id and syscolumns.name = '%s'",
            $this->prefix,
            $table_name,
            $column_name
        );
    ...
    {
        $query = sprintf(
            "select sysindexes.name as name from sysindexes, sysobjects where sysobjects.name = '%s%s' and sysobjects.id = sysindexes.id and sysindexes.name = '%s'",
            $this->prefix,
            $table_name,
            $index_name
        );
    ...
    function _executeSelectAct($queryObject, $connection = null)
    {
        ...
        $query .= (__DEBUG_QUERY__ & 1 && $output->query_id) ? sprintf(
            ' ' . $this->comment_syntax,
            $this->query_id
        ) : '';
