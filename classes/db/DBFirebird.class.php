...
class DBFireBird extends DB
{
    ...
    function DBFireBird(LoggerInterface $logger = null)
    {
        ...
        if (!$connected) {
            throw new DBConnectionFailedException ("DB connection failed!");
        }
    }
    ...
    function __connect($connection)
    {
        ...
        if (($service = ibase_service_attach(
    ...
    function autoQuotes($string)
    {
        ...
        if (strpos($string, "substr(") !== false) {
            ...
            return sprintf(
                ...
                $this->addDoubleQuotes($tokkens[0]),
                $tokkens[1],
                $tokkens[2]
            );
        }
    ...
    {
        $query = sprintf(
            "select rdb\$relation_name from rdb\$relations where rdb\$system_flag=0 and rdb\$relation_name = '%s%s';",
            $this->prefix,
            $target_name
        );
    ...
    function addColumn($table_name, $column_name, $type = 'number', $size = '', $default = '', $notnull = false)
    {
        ...
        if (strtoupper($type) == 'INTEGER') {
            ...
            if (strtoupper($type) == 'BIGINT') {
                $size = null;
            } else {
                if (strtoupper($type) == 'BLOB SUB_TYPE TEXT SEGMENT SIZE 32') {
                    $size = null;
                } else {
                    if (strtoupper($type) == 'VARCHAR' && !$size) {
                        $size = 256;
                    }
                }
            }
    ...
    {
        $query = sprintf(
            "SELECT RDB\$FIELD_NAME as \"FIELD\" FROM RDB\$RELATION_FIELDS WHERE RDB\$RELATION_NAME = '%s%s'",
            $this->prefix,
            $table_name
        );
        $result = $this->_query($query);
    ...
    function isIndexExists($table_name, $index_name)
    {
        ...
        for ($i = 0; $i < count($output); $i++) {
            if (trim($output[$i]->KEY_NAME) == $index_name) {
                return true;
            }
        }
    ...
    function _createTable($xml_doc)
    {
        ...
        foreach ($columns as $column) {
            ...
            if ($this->column_type[$type] == 'INTEGER') {
                ...
                if ($this->column_type[$type] == 'BIGINT') {
                    $size = null;
                } else {
                    if ($this->column_type[$type] == 'BLOB SUB_TYPE TEXT SEGMENT SIZE 32') {
                        $size = null;
                    } else {
                        if ($this->column_type[$type] == 'VARCHAR' && !$size) {
                            $size = 256;
                        }
                    }
                }
    ...
    function queryPageLimit($queryObject, $result, $connection)
    {
        ...
        if ($limit && $limit->isPageHandler()) {
            ...
            $count_query .= (__DEBUG_QUERY__ & 1 && $output->query_id) ? sprintf(
                ' ' . $this->comment_syntax,
                $this->query_id
            ) : '';
