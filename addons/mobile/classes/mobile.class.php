...
class mobileXE
{
    ...
    function &getInstance()
    {
        ...
        if (!$instance) {
            ...
            $this->lang = FileHandler::readFile(
                './files/cache/addons/mobile/setLangType/personal_settings/' . md5(
                    trim($_SERVER['HTTP_USER_AGENT']) . trim($_SERVER['HTTP_PHONE_NUMBER']) . trim(
                        $_SERVER['HTTP_HTTP_PHONE_NUMBER']
                    )
                ) . '.php'
            );
            if ($this->lang) {
    ...
    function setModuleInstance(&$oModule)
    {
        ...
        if ($cur_menu_item['parent_srl']) {
            ...
            if ($parent_srl && $listed_items[$parent_srl]) {
                ...
                if ($parent_item) {
                    $this->setUpperUrl(
                        getUrl('', 'mid', $parent_item['mid']),
                        Context::getLang('cmd_go_upper')
                    );
                }
            }
        } elseif (!$this->isNavigationMode()) {
            $this->setUpperUrl(
                getUrl('', 'mid', $this->index_mid, 'nm', '1', 'cmid', 0),
                Context::getLang('cmd_view_sitemap')
            );
        }
    }
    ...
    public static function getBrowserType()
    {
        ...
        if (preg_match("/SKT11/i", $userAgent) || preg_match("/skt/i", $browserAccept)) {
            ...
            return "wml";
        } elseif (preg_match("/hdml/i", $browserAccept)) {
            return "hdml";
        } elseif (preg_match(
            "/CellPhone/i",
            $userAgent
