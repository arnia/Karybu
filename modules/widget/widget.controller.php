...
class widgetController extends widget
{
    ...
    function procWidgetGenerateCodeInPage()
    {
        ...
        if (!in_array($widget, array('widgetBox', 'widgetContent')) && !Context::get(
            'skin'
        )
        ) {
            return new Object(-1, Context::getLang('msg_widget_skin_is_null'));
    ...
    function procWidgetCopyDocument()
    {
        ...
        $output = $oDocumentAdminController->copyDocumentModule(
            array($oDocument->get('document_srl')),
            $oDocument->get('module_srl'),
            0
        );
        if (!$output->toBool()) {
            return $output;
    ...
    function transWidgetCode($content, $javascript_mode = false, $isReplaceLangCode = true)
    {
        ...
        $content = preg_replace_callback(
            '!<div([^\>]*)widget=([^\>]*?)\><div><div>((<img.*?>)*)!is',
            array($this, 'transWidgetBox'),
            $content
        );
            ...
            switch ($widget) {
                ...
                case 'widgetContent' :
                    ...
                    $widget_content_header = sprintf(
                        '<div %sstyle="overflow:hidden;%s"><div style="%s">',
                        $args->id,
                        $style,
                        $inner_style
                    );
                ...
                case 'widgetBox' :
                    $widget_content_header = sprintf(
                        '<div %sstyle="overflow:hidden;%s;"><div style="%s"><div>',
                        $args->id,
                        $style,
                        $inner_style
                    );
