...
class documentModel extends document
{
    ...
    function setToAllDocumentExtraVars()
    {
        ...
        foreach ($GLOBALS['XE_DOCUMENT_LIST'] as $key => $val) {
            if (!$val->document_srl || isset($checked_documents[$val->document_srl])) {
                continue;
            }
        ...
        if ($output->toBool() && $output->data) {
            foreach ($output->data as $key => $val) {
                ...
                if (!$extra_vars[$val->module_srl][$val->document_srl][$val->var_idx][0]) {
                    $extra_vars[$val->module_srl][$val->document_srl][$val->var_idx][0] = trim(
                        $val->value
                    );
                }
        ...
        for ($i = 0, $c = count($document_srls); $i < $c; $i++) {
            ...
            if (!$GLOBALS['XE_DOCUMENT_LIST'][$document_srl] || !is_object(
                $GLOBALS['XE_DOCUMENT_LIST'][$document_srl]
            ) || !$GLOBALS['XE_DOCUMENT_LIST'][$document_srl]->isExists()
            ) {
                continue;
            }
            ...
            // Title Processing
            if (isset($vars[-1][$user_lang_code])) {
                $GLOBALS['XE_DOCUMENT_LIST'][$document_srl]->add(
                    'title',
                    $vars[-1][$user_lang_code]
                );
            }
            // Information processing
            if (isset($vars[-2][$user_lang_code])) {
                $GLOBALS['XE_DOCUMENT_LIST'][$document_srl]->add(
                    'content',
                    $vars[-2][$user_lang_code]
                );
            }

            if (isset($vars[-1][$user_lang_code]) || isset($vars[-2][$user_lang_code])) {
    ...
    function getDocumentMenu()
    {
        ...
        if ($logged_info->is_admin == 'Y') {
            ...
            if ($oDocument->isExists()) {
                ...
                $url = sprintf(
                    "var params = new Array(); params['ipaddress_list']='%s'; exec_xml('spamfilter', 'procSpamfilterAdminInsertDeniedIP', params, completeCallModuleAction)",
                    $oDocument->getIpAddress()
                );
