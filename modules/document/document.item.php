...
class documentItem extends Object
{
    ...
    function isGranted()
    {
        ...
        $grant = $oModuleModel->getGrant(
            $oModuleModel->getModuleInfoByModuleSrl($this->get('module_srl')),
            $logged_info
        );
        if ($grant->manager) {
            return true;
        ...
        if ($this->get('member_srl') && ($this->get('member_srl') == $logged_info->member_srl || $this->get(
            'member_srl'
        ) * -1 == $logged_info->member_srl)
        ) {
            return true;
        }
    ...
    function notify($type, $content)
    {
        ...
        $content = sprintf(
            '%s<br /><br />from : <a href="%s" target="_blank">%s</a>',
            $content,
            getFullUrl('', 'document_srl', $this->document_srl),
            getFullUrl('', 'document_srl', $this->document_srl)
        );
    ...
    function getContentText($strlen = 0)
    {
        ...
        $content = preg_replace_callback(
            '/<(object|param|embed)[^>]*/is',
            array($this, '_checkAllowScriptAccess'),
            $content
        );
    ...
    function getContent(
        ...
        if ($oContext->allow_rewrite) {
            $content = preg_replace(
                '/<a([ \t]+)href=("|\')\.\/\?/i',
                "<a href=\\2" . Context::getRequestUri() . "?",
                $content
            );
        }
        ...
        if ($add_popup_menu) {
            $content = sprintf(
                ...
                $content,
                $this->document_srl,
                Context::getLang('cmd_document_do')
            );
        }
    ...
    {
        return $this->getRegdate('Y-m-d') . 'T' . $this->getRegdate('H:i:s') . substr(
            $GLOBALS['_time_zone'],
            0,
            3
        ) . ':' . substr($GLOBALS['_time_zone'], 3, 2);
    }
    ...
    {
        return $this->getUpdate('Y-m-d') . 'T' . $this->getUpdate('H:i:s') . substr(
            $GLOBALS['_time_zone'],
            0,
            3
        ) . ':' . substr($GLOBALS['_time_zone'], 3, 2);
    ...
    function getComments()
    {
        ...
        foreach ($output->data as $key => $val) {
            ...
            if ($val->parent_srl > 0 && $val->is_secret == 'Y' && !$oCommentItem->isAccessible(
            ) && $accessible[$val->parent_srl] === true
            ) {
    ...
    function getThumbnail($width = 80, $height = 0, $thumbnail_type = '')
    {
        ...
        if ($source_file) {
            $output = FileHandler::createImageFile(
                $source_file,
                $thumbnail_file,
                $width,
                $height,
    ...
    function getExtraImages($time_interval = 43200)
    {
        ...
        if ($this->get('regdate') > $time_check) {
            ...
            if ($this->get('last_update') > $time_check) {
                $buffs[] = "update";
            }
        }
    ...
    function getCommentEditor()
    {
        ...
        $oEditorModel = & getModel('editor');
        return $oEditorModel->getModuleEditor(
            'comment',
            $this->get('module_srl'),
            $comment_srl,
            'comment_srl',
    ...
    function getSignature()
    {
        ...
        if ($signature) {
            ...
            if ($max_signature_height) {
                $signature = sprintf(
                    '<div style="max-height:%dpx;overflow:auto;overflow-x:hidden;height:expression(this.scrollHeight > %d ? \'%dpx\': \'auto\')">%s</div>',
    ...
    function _checkAccessibleFromStatus()
    {
        ...
        if ($status == $configStatusList['public'] || $status == $configStatusList['publish']) {
            ...
            if ($status == $configStatusList['private'] || $status == $configStatusList['secret']) {
