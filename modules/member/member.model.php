...
class memberModel extends member
{
    ...
    function getLoggedInfo()
    {
        ...
        if ($this->isLogged()) {
            ...
            if ($site_module_info->site_srl) {
                $logged_info->group_list = $this->getMemberGroups(
    ...
    function getMemberInfoByMemberSrl($member_srl, $site_srl = 0, $columnList = array())
    {
        ...
        if (!isset($GLOBALS['__member_info__'][$member_srl]) || count($columnList) == 0) {
            ...
            if (!isset($GLOBALS['__member_info__'][$member_srl])) {
                ...
                if ($oCacheHandler->isSupport()) {
                    $oCacheHandler->put(
                        $cache_key,
                        $GLOBALS['__member_info__'][$member_srl]
                    );
                }
            }
        }
    ...
    function getMemberGroups($member_srl, $site_srl = 0, $force_reload = false)
    {
        ...
        if (!isset($member_groups[$member_srl][$site_srl]) || $force_reload) {
            if (!isset($output)) {
                $args = new stdClass();
                $args->member_srl = $member_srl;
