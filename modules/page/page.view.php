...
class pageView extends page
{
    ...
    function executeFile($path, $caching_interval, $cache_file)
    {
        ...
        // Verify cache
        if ($caching_interval < 1 || !file_exists($cache_file) || filemtime(
            $cache_file
        ) + $caching_interval * 60 <= time() || filemtime($cache_file) < filemtime($path)
        ) {
    ...
    function _replacePath($matches)
    {
        ...
        if (strpos($val, '.') === false || preg_match(
            '@^((?:http|https|ftp|telnet|mms)://|(?:mailto|javascript):|[/#{])@i',
            $val
        )
        ) {
            return $matches[0];
