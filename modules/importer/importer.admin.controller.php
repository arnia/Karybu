    ...
    class importerAdminController extends importer {
            ...
            for($idx=$cur;$idx<$cur+$this->unit_count;$idx++) {
                ...
                if($output->toBool() && !($obj->password)){
                    ...
                    $oMail->setContent("Dear $obj->user_name, <br /><br />
                                            We recently migrated our phpBB forum to Karybu. Since you password was encrypted we could not migrate it too, so please reset it by following this link:
        ...
        function importModule($key, $cur, $index_file, $module_srl) {
            ...
            for($idx=$cur;$idx<$cur+$this->unit_count;$idx++) {
                ...
                $obj->last_update = base64_decode($xmlDoc->post->last_update->body);
