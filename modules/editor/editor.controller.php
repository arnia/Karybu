...
class editorController extends editor
{
    ...
    function triggerEditorComponentCompile(&$content)
    {
        ...
        if ($module_srl) {
            ...
            if ($content_style) {
                $path = _KARYBU_PATH_ . 'modules/editor/styles/' . $content_style . '/';
                if (is_dir($path) && file_exists($path . 'style.ini')) {
                    ...
                    for ($i = 0, $c = count($ini); $i < $c; $i++) {
                        ...
                        if (preg_match('/\.css$/i', $file)) {
                            Context::addCSSFile(
                                './modules/editor/styles/' . $content_style . '/' . $file,
                                false
                            );
    ...
    function transComponent($content)
    {
        $content = preg_replace_callback(
            '!<(?:(div)|img)([^>]*)editor_component=([^>]*)>(?(1)(.*?)</div>)!is',
            array($this, 'transEditorComponent'),
            $content
        );
        return $content;
    }
    ...
    function makeCache($filter_enabled = true, $site_srl)
    {
        ...
        $db_list = $output->data;
        // Get a list of files
        $downloaded_list = FileHandler::readDir(_KARYBU_PATH_ . 'modules/editor/components');
        ...
        foreach ($db_list as $component) {
            ...
            $component_icon_file = _KARYBU_PATH_ . 'modules/editor/components/' . $component_name . '/component_icon.gif';
        ...
        if ($filter_enabled) {
            ...
            $buff = sprintf(
                '<?php if(!defined("__KARYBU__")) exit(); $component_list = unserialize("%s"); ?>',
                str_replace('"', '\\"', serialize($component_list))
            );
