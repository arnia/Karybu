<?php
namespace Karybu\DependencyInjection;
...
class CMSContainer
{
    ...
    public function registerCMSContainer()
    {
        ...
        $this->register('cms.config.locator', 'Karybu\Config\ConfigLocator');
        $this->register('cms.router.loader', 'Karybu\Routing\Loader\YamlFileLoader')->setArguments(array(new Reference('cms.config.locator')));
        $this->register('context', 'Symfony\Component\Routing\RequestContext');
        $this->register('cms.router', 'Karybu\Routing\Router')->setArguments(array(new Reference('cms.router.loader'), new Reference('context'), null, '%debug%'));
        $this->register('cms.context.instance', 'ContextInstance')->setArguments(array(null, null, null, new Reference('cms.router')));

        $this->register('listener.router', 'Karybu\EventListener\RouterListener')->setArguments(array(new Reference('cms.router')));
        $this->register('listener.response', 'Symfony\Component\HttpKernel\EventListener\ResponseListener')->setArguments(array('%charset%'));
        $this->register('listener.exception', 'Karybu\EventListener\ExceptionListener')
            ->setArguments(array(new Reference("logger.exceptions")));
        $this->register('listener.cms', 'Karybu\EventListener\CMSListener')->setArguments(array(new Reference('cms.context.instance'), new Reference('logger')));

        $this->register('listener.debug.toolbar', 'Karybu\Module\Debug\EventListener\DebugToolbarListener')
        ...
        $this->register('listener.db.query_info', 'Karybu\EventListener\Debug\DBQueryInfoListener')
            ->setArguments(array(new Reference('logger.db_info')));
        $this->register('listener.db.slow_query', 'Karybu\EventListener\Debug\SlowQueryListener')
            ->setArguments(array('%log_slow_query_min_duration%', new Reference('logger.db_slow_query')));
        $this->register('listener.db.errors', 'Karybu\EventListener\Debug\QueryErrorListener')
            ->setArguments(array(new Reference('logger.db_errors')));
        $this->register('listener.error.handler', 'Karybu\EventListener\ErrorHandler');

        // listener around Response, used to aggregate summary statistics
        $this->register('listener.response.summary', 'Karybu\EventListener\Debug\ResponseSummaryInfoListener')
        ...
        $this->register('resolver', 'Karybu\HttpKernel\Controller\ControllerResolver');
        $this->register('http_kernel', 'Karybu\HttpKernel\HttpKernel')->setArguments(array(new Reference('dispatcher'), new Reference('resolver')));
    }
